...
.header { text-align: center; background: white; border-radius: 12px; padding: 20px; margin-bottom: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
.header h1 { color: var(--primary-color); font-size: 1.8em; margin-bottom: 8px; }
.header .meta { color: #666; font-size: 0.85em; }
.signal-box { display: inline-block; padding: 8px 20px; border-radius: 20px; font-weight: bold; font-size: 1em; margin: 10px 0; }
...
.metrics-bar { display: flex; justify-content: center; gap: 12px; flex-wrap: wrap; margin: 12px 0; }
.metric-badge { background: var(--primary-color); color: white; padding: 6px 14px; border-radius: 16px; font-size: 0.8em; }
.metric-badge .value { font-weight: bold; }
.tab-nav { display: flex; gap: 4px; background: white; padding: 8px; border-radius: 12px 12px 0 0; box-shadow: 0 2px 8px rgba(0,0,0,0.1); overflow-x: auto; }
.tab-btn { flex: 1; min-width: 90px; padding: 10px 12px; border: none; background: var(--secondary-color); color: #666; font-size: 0.85em; font-weight: 500; cursor: pointer; border-radius: 6px; transition: all 0.3s ease; white-space: nowrap; }
.tab-btn:hover { background: #e0e0e0; }
.tab-btn.active { background: var(--primary-color); color: white; }
.tab-content-wrapper { background: white; border-radius: 0 0 12px 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); min-height: 400px; }
...
@media (max-width: 600px) { .tab-nav { flex-wrap: wrap; } .tab-btn { min-width: 70px; font-size: 0.75em; padding: 8px 6px; } .metrics-bar { gap: 6px; } .metric-badge { padding: 5px 8px; font-size: 0.7em; } }
"""
...
<body>
    <div class="header">
        ...
        <div class="metrics-bar">
            <div class="metric-badge">RSI: <span class="value">{f"{metrics.get('rsi', 'N/A'):.2f}" if metrics.get('rsi') else 'N/A'}</span></div>
            <div class="metric-badge">MACD: <span class="value">{f"{metrics.get('macd', 'N/A'):.2f}" if metrics.get('macd') else 'N/A'}</span></div>
            <div class="metric-badge">技术信号: <span class="value">{metrics.get('signal', 'N/A')}</span></div>
            <div class="metric-badge">支撑: <span class="value">{f"{metrics.get('support', 'N/A'):.2f}" if metrics.get('support') else 'N/A'}</span></div>
            <div class="metric-badge">阻力: <span class="value">{f"{metrics.get('resistance', 'N/A'):.2f}" if metrics.get('resistance') else 'N/A'}</span></div>
        </div>
    </div>
    <div class="tab-nav">
    ...
    <div class="tab-content-wrapper">
        <div id="tab-market" class="tab-content active"><h2>📊 市场技术分析</h2>{convert_md(data.get('market_report'))}</div>
        <div id="tab-sentiment" class="tab-content"><h2>💬 情绪分析</h2>{convert_md(data.get('sentiment_report'))}</div>
        <div id="tab-news" class="tab-content"><h2>📰 新闻分析</h2>{convert_md(data.get('news_report'))}</div>
        <div id="tab-fundamentals" class="tab-content"><h2>📈 基本面分析</h2>{convert_md(data.get('fundamentals_report'))}</div>
        <div id="tab-debate" class="tab-content"><h2>🔄 投资辩论</h2><h3>看多方观点</h3>{convert_md(data.get('bull_history'))}<h3>看空方观点</h3>{convert_md(data.get('bear_history'))}<h3>研究经理决策</h3>{convert_md(data.get('investment_plan'))}</div>
        <div id="tab-trading" class="tab-content"><h2>💼 交易决策</h2><h3>交易员提案</h3>{convert_md(data.get('trader_investment_plan'))}</div>
        <div id="tab-risk" class="tab-content"><h2>⚖️ 风险辩论</h2><h3>激进观点</h3>{convert_md(data.get('aggressive_history'))}<h3>保守观点</h3>{convert_md(data.get('conservative_history'))}<h3>中性观点</h3>{convert_md(data.get('neutral_history'))}</div>
        <div id="tab-decision" class="tab-content"><h2>🎯 最终交易决策</h2>{convert_md(data.get('final_trade_decision'))}</div>
    </div>
    <div class="footer"><p>由 TradingAgents 多智能体框架生成 | 仅供参考，不构成投资建议</p></div>
    {JAVASCRIPT_TAB}
</body>
</html>"""
